 ...
 */
@TargetApi(18)
public final class DrmtodayHttpMediaDrmCallback implements MediaDrmCallback {
   ...
   */
  public DrmtodayHttpMediaDrmCallback(String defaultLicenseUrl, HttpDataSource.Factory dataSourceFactory) {
   ...
   * @param dataSourceFactory A factory from which to obtain {@link HttpDataSource} instances.
   */
  public DrmtodayHttpMediaDrmCallback(String defaultLicenseUrl, boolean forceDefaultLicenseUrl,
  ...
  public byte[] executeKeyRequest(UUID uuid, KeyRequest request) throws Exception {
    ...
    final byte[] bytes = executePost(dataSourceFactory, url, request.getData(), requestProperties);

    try {
        JSONObject jsonObject = new JSONObject(new String(bytes));
        String base64License = jsonObject.getString("license");
        if (base64License == null) {
            return bytes;
        }
        return Base64.decode(base64License, Base64.DEFAULT);
    } catch (JSONException e) {
        // throw new Exception("Error while parsing widevine response");
        return bytes;
    }
  }
